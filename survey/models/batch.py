...
#     group = models.ForeignKey("HouseholdMemberGroup", null=True, related_name="question_group")
    start_question = models.OneToOneField(
        Question, related_name='starter_batch', null=True, blank=True, on_delete=models.SET_NULL)
    ...
    def loop_starters(self):
        ...
        flows = QuestionFlow.objects.filter(
            next_question__batch=self, desc=LogicForm.BACK_TO_ACTION)
        return set([f.next_question for f in flows])

    def loop_enders(self):
        from survey.forms.logic import LogicForm
        flows = QuestionFlow.objects.filter(
            question__batch=self, desc=LogicForm.BACK_TO_ACTION)
    ...
    def next_inline(self, question, groups=ALL_GROUPS, channel=ODKAccess.choice_name(), exclude_groups=[]):
        qflows = QuestionFlow.objects.filter(
            question__batch=self, validation_test__isnull=True)
        if qflows.exists():
    ...
    def survey_questions(self):
        ...
        def _survey_questions():
            ...
            for ques in inline_ques:
                ...
                map(lambda q: survey_questions.add(
