...
class RandomizationCriterion(BaseModel):
    ...
    def test_arguments(self):
        return CriterionTestArgument.objects.filter(test_condition=self).order_by('position')
...
class ListingSample(BaseModel):
    ...
    def generate_random_samples(cls, from_survey, to_survey, ea):
        ...
        valid_interviews = from_survey.interviews.filter(ea=ea, # the listed interviews in the ea
                                                         interview__qset=from_survey.listing_form).values('id',
                                                                                                          flat=True)
        ...
        for criterion in to_survey.randomization_criteria.all():  # need to optimize this
            answer_class = Answer.get_class(criterion.listing_question.answer_type)
            data_sets = answer_class.objects.filter(question=criterion.listing_question,
                                                    interview__pk__in=valid_interviews).only('interview__id',
                                                                                             'value').values(
                'interview__id', 'value')
        ...
        random_samples = valid_interviews[:to_survey.listing_settings.sample_size]
        samples = []
