...
class Migration(DataMigration):
    ...
    def backwards(self, orm):
        ...
        for rule in orm['survey.answerrule'].objects.all():
            similar_rule = orm['survey.answerrule'].objects.filter(question=rule.question)
            rule_count = len(similar_rule)
            count = 0
            while count<(rule_count-1):
                orm['survey.answerrule'].objects.get(id=similar_rule[count].id).delete()
                count += 1
