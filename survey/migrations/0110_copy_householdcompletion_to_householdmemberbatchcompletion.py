...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        member_completion = HouseholdMemberBatchCompletion()
        fields = [str(item.attname) for item in member_completion._meta.fields]
        member_completion_data = {}

        for batch_completion in orm['survey.householdbatchcompletion'].objects.all():
            for field in fields:
                member_completion_data[field] = getattr(batch_completion, field)
            HouseholdMemberBatchCompletion.objects.create(**member_completion_data)

        orm['survey.householdbatchcompletion'].objects.all().delete()

    def backwards(self, orm):
        household_completion = HouseholdBatchCompletion()
        fields = [str(item.attname) for item in household_completion._meta.fields]
        household_completion_data = {}

        for batch_completion in orm['survey.householdmemberbatchcompletion'].objects.all():
            for field in fields:
                household_completion_data[field] = getattr(batch_completion, field)
            HouseholdBatchCompletion.objects.create(**household_completion_data)

        orm['survey.householdmemberbatchcompletion'].objects.all().delete()
