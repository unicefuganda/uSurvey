...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for group_condition in orm['survey.groupcondition'].objects.all():
            similar_conditions = orm['survey.groupcondition'].objects.filter(condition=group_condition.condition,
                                                                             attribute=group_condition.attribute, value=group_condition.value)
            condition_count = len(similar_conditions)
            count = 0
            while count<(condition_count-1):
                orm['survey.groupcondition'].objects.get(id=similar_conditions[count]).delete()
