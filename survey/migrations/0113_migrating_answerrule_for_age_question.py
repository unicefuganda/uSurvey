...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        age_question = orm["survey.question"].objects.filter(text__startswith="Please Enter the age")
        if age_question:
            modified_question = age_question[0]
            answer_rule = orm["survey.answerrule"].objects.filter(question=modified_question)
            if answer_rule:
                reset_rule = answer_rule[0]
                reset_rule.validate_with_value = 0
                reset_rule.save()
            modified_question.text = "Please Enter the age\n(enter 0 if not known)"
