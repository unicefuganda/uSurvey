...
class SurveyForm(ModelForm, FormOrderMixin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if kwargs.get(
            'instance',
                None) and kwargs['instance'].has_sampling is False:
    ...
    def clean_random_sample_label(self):
        ...
        :return:
        """
        if self.cleaned_data['has_sampling'] and not self.data.get(
            ...
            raise ValidationError(
                '%s is not in %s' %
                (', '.join(requested_identifiers), listing_form.name))
    ...
    def clean_listing_form(self):
        if self.cleaned_data.get('has_sampling', False) and not (self.cleaned_data.get(
                'listing_form') or self.cleaned_data.get('preferred_listing')):
            raise ValidationError(
                'Chose Either listing_form/preferred_listing ')
        if self.cleaned_data.get(
                'listing_form') and self.cleaned_data.get('preferred_listing'):
            raise ValidationError(
                'Chose Either listing_form/preferred_listing ')
