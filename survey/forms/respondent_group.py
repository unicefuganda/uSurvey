...
class GroupForm(forms.ModelForm, FormOrderMixin):
    ...
    def save(self, *args, **kwargs):
        ...
        if validation_test and test_question:
            group_condition = group.group_conditions.create(validation_test=validation_test,
                                                            test_question=test_question)
            if validation_test == 'between':
                group_condition.arguments.create(position=0, param=self.cleaned_data['min'])
                group_condition.arguments.create(position=1, param=self.cleaned_data['max'])
            else:
                group_condition.arguments.create(position=0, param=self.cleaned_data['value'])
        return group
