...
class UploadService(object):
    ...
    def clean_db(self):
        one_month_before_today = datetime.utcnow().replace(
            tzinfo=utc) - timedelta(days=UPLOAD_ERROR_LOG_EXPIRY)
        all_entries_before_one_month = UploadErrorLog.objects.filter(
            model=self.MODEL, created__lte=one_month_before_today)
        all_entries_before_one_month.delete()

    def log_error(self, row_number, error):
        UploadErrorLog.objects.create(model=self.MODEL,
                                      filename=self.file.name[:19],
    ...
    def remove_trailing(name, in_array, exclude=None):
        ...
        if exclude:
            new_array = filter(
                lambda element: not element.endswith(exclude), new_array)
        return new_array
