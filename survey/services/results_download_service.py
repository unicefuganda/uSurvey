...
class ResultsDownloadService(object):
    ...
    def _set_survey_and_questions(self, survey):
        ...
        survey_questions = []
        map(lambda batch: survey_questions.extend(
            list(batch.survey_questions)), survey.batches.all())
        return survey, survey_questions

    def set_report_headers(self):
        header = [loc.name for loc in LocationType.objects.exclude(
            name__iexact="country")]

        other_headers = ['EA', 'Household Number', 'Family Name',
                         'First Name',  'Age', 'Date of Birth', 'Gender']
    ...
    def get_summarised_answers(self):
        ...
        if self.specific_households is None:
            ...
            all_households = Household.objects.filter(pk__in=self.specific_households,
                                                      listing__survey_houselistings__survey=self.survey)
        locations = list(set(all_households.values_list(
            'listing__ea__locations', flat=True)))
        for location_id in locations:
            ...
            location_ancestors = household_location.get_ancestors(include_self=True).\
                exclude(parent__isnull='country').values_list(
                    'name', flat=True)
            answers = []
                ...
                for member in household.members.all():
                    try:
                        ...
                        answers.extend([household.listing.ea.name, household.house_number, '%s-%s' %
                                        (member.surname, member.first_name), str(
                                            member.age),
        ...
        for answer_type in Answer.answer_types():
            ...
            for data in answer_data:
                ...
                if hm_data is None:
                    ...
                    member_details = [unicode(md).encode(
                        'utf8') for md in report_data[:-1]]
    ...
    def _get_ancestors_names(self, household_location, exclude_type='country'):
        location_ancestors = get_ancestors(
            household_location, include_self=True)
        if exclude_type:
