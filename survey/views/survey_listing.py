...
def index(request):
    return QuestionSetView(
        model_class=ListingTemplate).index(
        request, ListingTemplate.objects.all())
...
def edit(request, qset_id):
    ...
    return QuestionSetView(
        model_class=ListingTemplate).edit(
        request,
        ListingTemplate.get(
            pk=qset_id))
...
def delete(request, qset_id):
    return QuestionSetView(
        model_class=ListingTemplate).delete(
        request, ListingTemplate.get(
            pk=qset_id))
