...
def permission_required_for_perm_or_current_user(perm, login_url=None):
    def check_perms(user, **kwargs):
        ...
        if user.has_perm(perm) or str(user_id) == str(user.id):
            return True
        return False
    return _modified_django_auth_user_passes_test(
        check_perms, login_url=login_url)


def _modified_django_auth_user_passes_test(
        test_func,
        login_url=None,
    ...
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            if test_func(
                    request.user,
                    **kwargs):  # changed to get kwargs as well
                return view_func(request, *args, **kwargs)
            messages.error(
                request, "Current user, %s, is not allowed\
                to perform this action. "
                "Please log in a user with enough privileges." %
        ...
        def _wrapped_view(request, *args, **kwargs):
            if 'batch_id' in kwargs and batch_is_open(kwargs['batch_id']):
                ...
                return HttpResponseRedirect(
                    reverse(redirect_url_name, kwargs=_kwargs))
            return view_func(request, *args, **kwargs)
        return _wrapped_view
    return decorator


def not_allowed_when_batch_is_open(
        message="This function is not allowed when batch is open.",
        redirect_url_name="login_page",
        url_kwargs_keys=None):
    def batch_is_open(batch_id):
        return BatchLocationStatus.objects.filter(batch=batch_id).exists()
    return batch_passes_test(
        batch_is_open,
        message,
        redirect_url_name,
        url_kwargs_keys)
