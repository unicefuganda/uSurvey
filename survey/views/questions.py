...
from survey.models import Batch, QuestionTemplate, QuestionFlow, TextArgument,\
    TemplateOption, QuestionSet
...
ADD_LOGIC_ON_OPEN_BATCH_ERROR_MESSAGE = "Logics cannot be \
    added while the batch is open."
...
def _max_number_of_question_per_page(number_sent_in_request):
    ...
    given_max_per_page = min(
        int(max_question_per_page_supplied),
        MAX_NUMBER_OF_QUESTION_DISPLAYED_PER_PAGE)
    return max(
        given_max_per_page,
        DEFAULT_NUMBER_OF_QUESTION_DISPLAYED_PER_PAGE)
...
def delete_logic(request, flow_id):
    ...
    return HttpResponseRedirect('/qsets/%s/questions/' % batch.id)
...
def edit(request, question_id):
    ...
    response, context = _render_question_view(
        request, QuestionSet.objects.get_subclass(
            pk=question.qset.pk), question)
...
def _remove(request, batch_id, question_id):
    ...
    if question.total_answers() > 0:
        messages.error(
            request,
            "Cannot delete question that has\
