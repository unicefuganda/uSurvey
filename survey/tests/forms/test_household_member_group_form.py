...
class HouseholdMemberGroupTests(TestCase):
    ...
    def setUp(self):
        self.group_condition = GroupCondition.objects.create(
            condition='haha', value="True", attribute="hmm")
        self.group_condition_2 = GroupCondition.objects.create(
            condition='another', value="True", attribute="hmm")
        self.form_data = {
            'name': "5 to 12 years",
            'order': 1,
    ...
    def test_invalid_order_on_add_group_if_group_already_exists(self):
        ...
        self.assertEqual(household_member_group_form.errors[
                         'order'][0], expected_form_error)
    ...
    def test_invalid_given_empty_fields_present(self):
        ...
        [self.assertTrue(household_member_group_form.errors.has_key(
            form_key)) for form_key in form_field_keys]
