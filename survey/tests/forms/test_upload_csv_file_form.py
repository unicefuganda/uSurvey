...
from survey.forms.upload_csv_file import UploadCSVFileForm
...
class UploadCSVFileFormTest(BaseTest):
    ...
    def test_not_csv_file(self):
        ...
        upload_location_form = UploadCSVFileForm(
            UploadLocation, {}, {'file': SimpleUploadedFile(self.filename, file.read())})
        self.assertEqual(False, upload_location_form.is_valid())
        self.assertIn('%s is not a valid csv file.' %
                      self.filename, upload_location_form.errors['file'])
