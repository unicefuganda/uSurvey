...
class HouseholdViewTest(BaseTest):
    ...
    def setUp(self):
        ...
        self.assign_permission_to(User.objects.create_user(
            'Rajni', 'rajni@kant.com', 'I_Rock'), 'can_view_households')
    ...
    def test_new_GET(self):
        ...
        investigator_form = {'value': '', 'text': '',
                             'options': Interviewer.objects.all(), 'error': ''}
...
class EditHouseholdDetailsTest(BaseTest):
    ...
    def setUp(self):
        ...
        self.city = LocationType.objects.create(
            name="City", parent=self.country, slug="city")
        ...
        self.kampala = Location.objects.create(
            name="Kampala", type=self.city, parent=self.uganda)
        ...
        LocationTypeDetails.objects.create(
            country=self.uganda, location_type=self.city)
        LocationTypeDetails.objects.create(
            country=self.uganda, location_type=self.village)
    ...
    def test_get_edit__household_details(self):
        ...
        self.assertIn('households/new.html', templates)
        self.assertEquals(
            response.context['action'], '/households/%s/edit/' % self.household.id)
        self.assertEquals(response.context['id'], 'create-household-form')
        self.assertEquals(response.context['button_label'], 'Save')
