...
class SetQuestionViewTest(BaseTest):
    ...
    def test_add_subquestion(self):
        ...
        q_obj = Question.objects.create(identifier='identifiersss', text="This is a questiod", answer_type='Numerical Answer',
                                                  qset_id=l_qset.id, response_validation_id=1)
        self.sub_data['parent_question'] = q_obj
        response = self.client.post(reverse('add_qset_subquestion_page',kwargs={"batch_id" : l_qset.id}), self.sub_data, HTTP_X_REQUESTED_WITH='XMLHttpRequest')
...
constraint_message="message")
        ...
        # qset = QuestionSet.get(pk=list_1.id)
        self.question_1 = Question.objects.create(identifier='id_1', text="This is a question 1111.1", answer_type='Numerical Answer',
