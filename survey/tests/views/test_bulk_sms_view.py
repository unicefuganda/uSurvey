...
class BulkSMSTest(BaseTest):
    ...
    def setUp(self):
        ...
        self.abim = Location.objects.create(
            name="Abim", type=self.district, parent=self.uganda)
    ...
    def test_send(self):
        response = self.client.post(
            '/bulk_sms/send', data={'locations': [self.kampala.pk, self.abim.pk], 'text': 'text'}, follow=True)
        self.assertEquals(len(response.context['messages']), 1)
        for message in response.context['messages']:
            self.assertEquals(
                str(message), "Your message has been sent to interviewers.")
        self.failUnlessEqual(response.status_code, 200)
        self.assertRedirects(response, 'http://testserver/bulk_sms')

    def test_send_failures(self):
        response = self.client.post(
            '/bulk_sms/send', data={'locations': [], 'text': 'text'}, follow=True)
        self.assertEquals(len(response.context['messages']), 1)
        for message in response.context['messages']:
        ...
        for message in response.context['messages']:
            self.assertEquals(
                str(message), "Please enter the message to send.")
        self.failUnlessEqual(response.status_code, 200)
        self.assertRedirects(response, 'http://testserver/bulk_sms')
