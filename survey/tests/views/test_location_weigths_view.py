...
class UploadWeightsTest(BaseTest):
    ...
    def test_should_render_context_data(self):
        ...
        self.assertEqual(response.context['id'],
                         "upload-location-weights-form")
        self.assertEqual(len(response.context['location_types']), 3)
        expected_types = [self.reqion_type,
                          self.district_type, self.county_type]
        [self.assertIn(_type, response.context['location_types'])
         for _type in expected_types]
        self.assertIsInstance(
            response.context['upload_form'], UploadWeightsForm)
...
class UploadWeightsErrorLogTest(BaseTest):
    ...
    def setUp(self):
        ...
        district = Location.objects.create(
            name="district1", parent=region, type=self.district_type)
        self.filename = 'test_uganda.csv'
        self.filedata = [['RegionName', 'DistrictName', 'CountyName', 'Selection Probability'],
                         ['region1',  'district1', 'county1', '0.01'],
                         ['region2', 'district2', 'county2', '0.1']]
    ...
    def test_should_get_list_and_returns_success_with_template(self):
        error_log = UploadErrorLog.objects.create(
            model="WEIGHTS", error="Some error", filename="Some file", row_number=1)
        error_log_1 = UploadErrorLog.objects.create(
            model="LOCATION", error="Some error", filename="Some file", row_number=25)
        error_log_2 = UploadErrorLog.objects.create(
            model="WEIGHTS", error="Some error_2", filename="Some file", row_number=25)
        ...
        [self.assertIn(error, response.context['error_logs'])
    ...
    def test_should_filter_to_and_from_dates(self):
        ...
        error_log_2 = UploadErrorLog.objects.create(
            model="WEIGHTS", error="Some error_2", filename="Some file", row_number=25)
        error_log_2.created = error_log_2.created.replace(
            tzinfo=utc) + datetime.timedelta(days=4)
        error_log_2.save()
        error_log_created = error_log.created.strftime('%Y-%m-%d')
        response = self.client.get(
            '/locations/weights/error_logs/?from_date=%s&to_date=%s' % (error_log_created, error_log_created))
        self.assertEqual(200, response.status_code)
        self.assertEqual(0, response.context['error_logs'].count())
        self.assertEqual(error_log.created.replace(
            hour=0, minute=0, second=0, microsecond=0), response.context['selected_from_date'])
        self.assertEqual(error_log.created.replace(
            hour=0, minute=0, second=0, microsecond=0), response.context['selected_to_date'])
