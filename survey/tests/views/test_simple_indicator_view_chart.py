...
class SimpleIndicatorChartViewTest(BaseTest):
    ...
    def setUp(self):
        ...
        self.west = Location.objects.create(name="WEST", type=self.district, parent=self.uganda)
        self.central = Location.objects.create(name="CENTRAL", type=self.district, parent=self.uganda)
        self.kampala = Location.objects.create(name="Kampala", parent=self.central, type=self.village)
        self.mbarara = Location.objects.create(name="Mbarara", parent=self.west, type=self.village)
    ...
    def test_get_data_for_simple_indicator_chart(self):
        ...
        region_data_series = [{'data': [0, 0], 'name': 'Yes'}, {'data': [0, 0], 'name': 'No'}]
    ...
    def test_get_data_for_simple_indicator_chart_for_the_second_hierarchy_level(self):
        ...
        response = self.client.get(url)
        self.failUnlessEqual(response.status_code, 200)
        self.assertEquals(response.context['location_names'], ['CENTRAL', 'WEST'])
        ...
        region_data_series = [{'data': [0, 0], 'name': self.yes_option.text}, {'data': [0, 0], 'name':self.no_option.text}]
        self.assertEquals(response.context['data_series'], region_data_series)
