...
class ExcelDownloadTest(BaseTest):
    ...
    def setUp(self):
        ...
        self.question_1 = Question.objects.create(batch=self.batch, text="How many members are there in this household?", answer_type=Question.NUMBER, order=1, identifier="QUESTION_1", group=self.member_group)
        self.question_2 = Question.objects.create(batch=self.batch, text="How many members are there in this household?", answer_type=Question.MULTICHOICE, order=2, identifier="QUESTION_2", group=self.member_group)
        ...
        sub_question_1 = Question.objects.create(batch=self.batch, text="Describe the source of drinking water", answer_type=Question.TEXT, subquestion=True, parent=self.question_2, group=self.member_group)

        self.question_3 = Question.objects.create(batch=self.batch, text="How many of them are male?", answer_type=Question.TEXT, order=3, identifier="QUESTION_3", group=self.member_group)
        ...
        self.investigator.member_answered(self.question_1, self.household_head, answer=1)
        self.investigator.member_answered(self.question_2, self.household_head, answer=1)
        self.investigator.member_answered(self.question_3, self.household_head, answer="ANSWER")
