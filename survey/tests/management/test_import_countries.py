...
class ImportCountriesTest(BaseTest):
    def setUp(self):
        ...
        LocationTypeDetails.objects.create(location_type=self.country, required=True, has_code=False)
    ...
    def test_should_create_country_type_if_not_existing_and_not_recreate_country_types_if_exists(self):
        ...
        self.assertEqual(1, LocationType.objects.filter(name="Country").count())
        self.country.delete()
        self.importer.handle(self.filename)
