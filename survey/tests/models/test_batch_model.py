...
class BatchTest(TestCase):
    ...
    def test_knows_batch_is_complete_if_completion_object_exists_for_member(self):
        ...
        self.assertEquals(mem.householdmember.surname,household_member.batch_completed(batch).householdmember.surname)
    ...
    def test_batch_knows_all_groups_it_has(self):
        ...
        household_member_group = HouseholdMemberGroup.objects.create(name="Greater than 65 years", order=10)
        question_mod = QuestionModule.objects.create(name="Test question name",description="test desc")
        question_1 = Question.objects.create(identifier='1.1',text="This is a question1", answer_type='Numerical Answer',
        ...
        [self.assertIn(batch_group, Question.objects.filter(batch=batch).values_list('group__name')) for batch_group in Question.objects.filter(batch=batch).values_list('group__name')]
...
class BatchLocationStatusTest(TestCase):
    ...
    def test_knows_whether_can_be_deleted(self):
        ...
        module = QuestionModule.objects.create(name="Test question name",description="test desc")
        question_1 = Question.objects.create(identifier='1.1',text="This is a question1", answer_type='Numerical Answer',
    ...
    def test_de_activate_non_response_for_location(self):
        ...
        self.assertEqual(0, batch.open_locations.filter(non_response=False, location=bukoto).count())
