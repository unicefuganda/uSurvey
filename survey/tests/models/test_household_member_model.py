...
#        fields_data = dict(surname='xyz', male=True, date_of_birth=date(1980, 05, 01), household=household)
    ...
    def test_household_members_count_per_location_in(self):
        ...
        kampala = Location.objects.create(name="Kampala1", type=district, parent=uganda)
        some_county = Location.objects.create(name="County1", type=county, parent=kampala)
        ...
        investigator = Interviewer.objects.create(name="Investigator",
                                                   ...
                                                   gender='1',level_of_education='Primary',
                                                   language='Eglish',weights=0)
        household_listing = HouseholdListing.objects.create(ea=ea1,list_registrar=investigator,initial_survey=survey)
        household = Household.objects.create(house_number=123456,listing=household_listing,physical_address='Test address',
                                             last_registrar=investigator,registration_channel="ODK Access",head_desc="Head",head_sex='MALE')
        survey_householdlisting = SurveyHouseholdListing.objects.create(listing=household_listing,survey=survey)
        household_member = HouseholdMember.objects.create(surname="sur", first_name='fir', gender='MALE', date_of_birth="1988-01-01",
        ...
        locations=[kampala,]
        self.assertEqual(kampala,household_member_group.household_members_count_per_location_in(locations,survey).keys()[0])
    ...
    def test_hierarchical_result_for(self):
        ...
        district = LocationType.objects.create(name="District11", parent=country,slug='district')
        county = LocationType.objects.create(name="County11", parent=district,slug='county')
        subcounty = LocationType.objects.create(name="SubCounty11", parent=county,slug='subcounty')
        parish = LocationType.objects.create(name="Parish11", parent=subcounty,slug='parish')
        village = LocationType.objects.create(name="Village11", parent=parish,slug='village')


        uganda = Location.objects.create(name="Uganda11", type=country)
        kampala = Location.objects.create(name="Kampala11", type=district, parent=uganda)
        some_county = Location.objects.create(name="County11", type=county, parent=kampala)
        ...
        household_member = HouseholdMember.objects.create(surname="sur11", first_name='fir11', gender='MALE', date_of_birth="1988-01-01",
                                                          household=household,survey_listing=survey_householdlisting,
                                                          registrar=investigator,registration_channel="ODK Access")
        household_member_group = HouseholdMemberGroup.objects.create(name="test name11", order=1)
        self.assertEqual(some_county,household_member_group.hierarchical_result_for(kampala,survey).keys()[0])
