...
from survey.models import Batch, Question, HouseholdMemberBatchCompletion, NumericalAnswer, UnknownDOBAttribute, MultiChoiceAnswer, QuestionOption, Survey, EnumerationArea
...
class HouseholdMemberTest(BaseTest):
    ...
    def test_should_have_fields_required(self):
        ...
        field_list_expected = ['id', 'created', 'modified', 'surname', 'first_name', 'gender', 'date_of_birth', 'household_id', 'survey_listing_id', 'registrar_id', 'registration_channel']
...
#     Eswar   household_member.age() error
...
#         self.county = LocationType.objects.create(name="County", slug=slugify("county"))
#         self.subcounty = LocationType.objects.create(name="Subcounty", slug=slugify("subcounty"))
#         self.parish = LocationType.objects.create(name="Parish", slug=slugify("parish"))
...
#         self.wakiso=Location.objects.create(name="Wakiso", tree_parent=self.west, type=self.district)
#         self.bujumba=Location.objects.create(name="Bujumba", tree_parent=self.wakiso, type=self.county)
#         self.bujumba1=Location.objects.create(name="Bujumba1", tree_parent=self.bujumba, type=self.subcounty)
#         self.buyama=Location.objects.create(name="Buyama", tree_parent=self.bujumba1, type=self.parish)
#         some_village = Location.objects.create(name="Some village",tree_parent=self.buyama ,type=self.village)
#         ea = EnumerationArea.objects.create(name="Kampala EA A")
#         #ea.locations.add(kampala)
#         investigator = Interviewer.objects.create(name="Investigator",
#                                                    ea=ea,
...
# #        fields_data = dict(surname='xyz', male=True, date_of_birth=date(1980, 05, 01), household=household)
#         survey_householdlisting = SurveyHouseholdListing.objects.create(listing=household_listing,survey=survey)
#         household_member = HouseholdMember.objects.create(surname="sur", first_name='fir', gender='MALE', date_of_birth=date(1980, 2, 2),
#                                                           household=household,survey_listing=survey_householdlisting,
#                                                           registrar=investigator,registration_channel="ODK Access")
#
#         date_today_that_makes_his_age_33 = date(2014, 04, 01)
#
#         self.assertEquals(str(household_member.age()), '36')
    #
...
# #        fields_data = dict(surname='xyz', male=True, date_of_birth=date(1980, 05, 01), household=household)
#         survey_householdlisting = SurveyHouseholdListing.objects.create(listing=household_listing,survey=survey)
#         household_member = HouseholdMember.objects.create(surname="sur", first_name='fir', gender='MALE', date_of_birth=date(1988,01,01),
#                                                           household=household,survey_listing=survey_householdlisting,
#                                                           registrar=investigator,registration_channel="ODK Access")
#
...
#
#         member_groups = household_member.groups()
...
#         self.assertIn(member_group_order_2, member_groups)
    ...
    #                                                language='Eglish',weights=0)
    #     household_listing = HouseholdListing.objects.create(ea=ea,list_registrar=investigator,initial_survey=survey)
    #     hhold = Household.objects.create(house_number=123456,listing=household_listing,physical_address='Test address',
    #                                          last_registrar=investigator,registration_channel="ODK Access",head_desc="Head",head_sex='MALE')
    #     survey_householdlisting = SurveyHouseholdListing.objects.create(listing=household_listing,survey=survey)
    #     household_member = HouseholdMember.objects.create(surname="sur", first_name='fir', gender='MALE', date_of_birth="1988-01-01",
    #                                                       household=hhold,survey_listing=survey_householdlisting,
    #                                                       registrar=investigator,registration_channel="ODK Access")
    #
    #
    #
    ...
    #     ea.locations.add(some_village)
    #
    #     investigator = Interviewer.objects.create(name="inv1", ea=ea)
    #
    ...
    #     investigator = Interviewer.objects.create(name="inv1")
    #
    #     member_group = HouseholdMemberGroup.objects.create(name="group1", order=0)
    #     member_group_2 = HouseholdMemberGroup.objects.create(name="group2", order=1)
    #
    ...
    #
    #     uganda = Location.objects.create(name="Uganda", type=country)
    #     investigator = Interviewer.objects.create(name="inv1", location=uganda,
    #                                                backend=Backend.objects.create(name='something'))
    #
    #     household = Household.objects.create(investigator=investigator, uid=0)
    #     household_member = HouseholdMember.objects.create(surname='member1', date_of_birth=(date(2013, 8, 30)),
