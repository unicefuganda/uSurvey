...
class IndicatorTest(TestCase):
    ...
    def test_fields(self):
        ...
        for field in ['id','created','modified','name','description','survey_id','question_set_id',
                      'display_on_dashboard','formulae']:
            self.assertIn(field, fields)

    def test_store(self):
        survey = Survey.objects.create(name="Survey")
        question_set = QuestionSet.objects.create(name="QuestionSet")
        indicator = Indicator.objects.create(name="indicator name", description="rajni indicator",
                                             survey=survey, formulae='indicatoir formule',
    ...
    def test_store(self):
        ...
        indicator = Indicator.objects.create(name="indicator name", description="rajni indicator",
                                             ...
                                             question_set_id=self.questionset.id, survey_id=self.survey.id)
        self.failUnless(indicator.id)
        self.failUnless(indicator.created)
        self.failUnless(indicator.description)
        self.failUnless(indicator.name)
    ...
    def test_unicode_text(self):
        survey = Survey.objects.create(name="Survey")
        question_set = QuestionSet.objects.create(name="QuestionSet")
        a = Indicator.objects.create(name="indicator name", description="rajni indicator", survey=survey,
                                     formulae='indicatoir formule', question_set=question_set)
