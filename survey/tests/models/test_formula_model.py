...
class FormulaTest(BaseTest):
    ...
    def create_household_member(self, household, survey_name):
        ...
        survey = Survey.objects.create(
            name=survey_name, description="Desc", sample_size=10, has_sampling=True)
    ...
    def test_compute_for_next_location_type_in_the_hierarchy(self):
        location_type_country = LocationType.objects.create(
            name="Country", slug='country')
        location_type_district = LocationType.objects.create(
            name="District", parent=location_type_country, slug='district')
        uganda = Location.objects.create(
            name="Uganda", type=location_type_country)
        kampala = Location.objects.create(
            name="Kampala", parent=uganda, type=location_type_district)
        abim = Location.objects.create(
            name="Abim", parent=uganda, type=location_type_district)
        ...
        formula = Formula.objects.create(numerator=self.question_1, groups=household_member_group, denominator=self.question_2,
                                         count=self.question_1, indicator=indicator)
        self.assertEquals(
            len(formula.compute_for_next_location_type_in_the_hierarchy(kampala)), 0)

    def test_get_group_formula_type(self):
        general_group = HouseholdMemberGroup.objects.create(
            name="GENERAL", order=21)
