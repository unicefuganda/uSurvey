...
class MultiChoiceQuestionSimpleIndicatorServiceTest(BaseTest):
    ...
    def setUp(self):
        ...
        self.household_head_1 = create_household_head(
            0, self.investigator, self.household_listing, self.survey_householdlisting)
        self.household_head_2 = create_household_head(
            1, self.investigator, self.household_listing, self.survey_householdlisting)
        self.household_head_3 = create_household_head(
            2, self.investigator, self.household_listing, self.survey_householdlisting)
        self.household_head_4 = create_household_head(
            3, self.investigator, self.household_listing, self.survey_householdlisting)
        self.household_head_5 = create_household_head(
            4, self.investigator, self.household_listing, self.survey_householdlisting)

        self.household_head_6 = create_household_head(
            5, self.investigator_2, self.household_listing, self.survey_householdlisting)
        self.household_head_7 = create_household_head(
            6, self.investigator_2, self.household_listing, self.survey_householdlisting)
        self.household_head_8 = create_household_head(
            7, self.investigator_2, self.household_listing, self.survey_householdlisting)
        self.household_head_9 = create_household_head(
            8, self.investigator_2, self.household_listing, self.survey_householdlisting)

    def test_gets_location_names_and_data_series_for_a_parent_location_and_formula(self):
        simple_indicator_service = SimpleIndicatorService(
            self.formula, self.uganda)
        region_responses = [{'data': [0, 0], 'name': self.yes_option.text}, {
    ...
    def test_formats_details_data(self):
        kibungo = Location.objects.create(
            name="Kibungo", type=self.district, parent=self.west)
        mpigi = Location.objects.create(
            name="Mpigi", type=self.district, parent=self.central)
        table_row_1 = {'Region': self.central.name, 'District': self.kampala.name,
                       self.yes_option.text: 3, self.no_option.text: 2, 'Total': 5}
        table_row_2 = {'Region': self.central.name, 'District': mpigi.name,
                       self.yes_option.text: 0, self.no_option.text: 0, 'Total': 0}
        table_row_3 = {'Region': self.west.name, 'District': kibungo.name,
                       self.yes_option.text: 0, self.no_option.text: 0, 'Total': 0}
        table_row_4 = {'Region': self.west.name, 'District': self.mbarara.name,
                       self.yes_option.text: 2, self.no_option.text: 2, 'Total': 4}
        expected_table_data = [table_row_1,
                               table_row_2, table_row_3, table_row_4]
...
class GroupCountSimpleIndicatorServiceTest(BaseTest):
    ...
    def setUp(self):
        ...
        self.general_group = HouseholdMemberGroup.objects.create(
            name="GENERAL", order=2)

        general_condition = GroupCondition.objects.create(
            attribute="GENERAL", value="HEAD", condition='EQUALS')
        ...
        self.option1 = QuestionOption.objects.create(
            question=self.question_1, text="This is an option1", order=1)
        self.option2 = QuestionOption.objects.create(
            question=self.question_2, text="This is an option2", order=2)
    ...
    def test_formats_details_data(self):
        ...
        mpigi = Location.objects.create(
            name="Mpigi", type=self.district, parent=self.central)
        table_row_1 = {'Region': self.central.name,
                       'District': self.kampala.name, self.general_group.name: 5, 'Total': 5}
        table_row_2 = {'Region': self.central.name,
                       'District': mpigi.name, self.general_group.name: 0, 'Total': 0}
        table_row_3 = {'Region': self.west.name, 'District': kibungo.name,
                       self.general_group.name: 0, 'Total': 0}
        table_row_4 = {'Region': self.west.name, 'District': self.mbarara.name,
                       self.general_group.name: 4, 'Total': 4}
        expected_table_data = [table_row_1,
                               table_row_2, table_row_3, table_row_4]
