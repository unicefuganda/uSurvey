...
class ResultsDownloadServiceTest(BaseTest):
    ...
    def setUp(self):
        ...
        LocationTypeDetails.objects.create(
            country=uganda, location_type=self.country)
        LocationTypeDetails.objects.create(
            country=uganda, location_type=self.district)
        LocationTypeDetails.objects.create(
            country=uganda, location_type=self.county)
        LocationTypeDetails.objects.create(
            country=uganda, location_type=self.subcounty)
        LocationTypeDetails.objects.create(
            country=uganda, location_type=self.parish)
        LocationTypeDetails.objects.create(
            country=uganda, location_type=self.village)
        ...
        general_condition = GroupCondition.objects.create(
            attribute="GENDER", value=False, condition='EQUALS')
    ...
    def test_gets_summarised_response_for_a_given_batch(self):
        ...
        group_1 = HouseholdMemberGroup.objects.create(
            name="Group 11", order=10)
        group_2 = HouseholdMemberGroup.objects.create(
            name="Group 12", order=11)
        group_3 = HouseholdMemberGroup.objects.create(
            name="Group 13", order=12)
        household_member_group = HouseholdMemberGroup.objects.create(
            name="Greater than 65 years", order=19)
    ...
    def test_should_repeat_questions_in_general_for_all_members(self):
        ...
        household_head_1 = create_household_head(
            0, self.investigator, household_listing1, survey_householdlisting1)
        household_head_2 = create_household_head(
            1, self.investigator, household_listing1, survey_householdlisting1)
        household_listing = HouseholdListing.objects.create(
    ...
    def test_gets_summarised_response_for_all_batches_under_survey(self):
        ...
        batchB = Batch.objects.create(
            order=2, name="different batch", survey=self.survey)
        module = QuestionModule.objects.create(
            name="Education in a different batch")
        ...
        yes_option = QuestionOption.objects.create(
            question=question_2B, text="Yes", order=1)
        no_option = QuestionOption.objects.create(
            question=question_2B, text="No", order=2)
